# ARKAN Theme — Developer Guide

> Complete development guide for contributing to and customizing the ARKAN theme.
...
bench get-app arkan_theme <git-url>
...
└── public/                     # All client-side assets
    ...
    │   ├── arkan_theme.js     # Main coordinator
    │   ├── arkan_skyline.js   # SVG skyline renderer
    │   ├── arkan_effects.js   # Canvas effects
    │   ├── arkan_splash.js    # Splash screen
    │   ├── arkan_loading.js   # Loading indicator
    │   ├── arkan_navbar.js    # Navbar enhancements
    │   └── arkan_login.js     # Login enhancements
    │
...
The import order in `arkan.scss` matters for cascading:
...
.my-element {
    background: $arkan-navy;           // Compiled to #1D2939
    color: var(--arkan-cream);          // Resolved at runtime
    transition: $arkan-transition-base;  // Compiled to "250ms ease"
}
...
```scss
.glassmorphism-panel {
    @include arkan-glass;           // Glass background + blur
}
...
.highlighted-element {
    @include arkan-gold-glow;       // Gold box-shadow glow
}

input:focus {
    @include arkan-focus-ring;      // Gold focus ring (accessibility)
}
```
...
1. **Always use ES5 syntax** — Frappe's build system expects ES5 for `app_include_js`
   ...
   > **Exception:** `arkan_skyline.js` and `arkan_effects.js` use some ES6 (`const`, `let`, arrow functions in non-critical paths). These work in modern browsers but consider keeping new code ES5 for maximum compatibility.
...
// ─── Session ───
frappe.boot              // All boot data including arkan_theme
...
1. `arkan_theme.js` — Sets up `window.arkan` namespace and config
2. `arkan_skyline.js` — SVG renderer (defines fallback helpers)
3. `arkan_effects.js` — Canvas effects
4. `arkan_splash.js` — Splash screen (uses `arkan.neuralGrid.create()`)
5. `arkan_loading.js` — Loading overlay (overrides `frappe.dom.freeze`)
6. `arkan_navbar.js` — Search overlay

> **Important:** `arkan_skyline.js` must load before `arkan_splash.js` and `arkan_login.js` because they call `arkan.neuralGrid.create()`.
...
1. Create `public/js/arkan_<name>.js` following the module pattern above
2. Add to `hooks.py` `app_include_js` (or `web_include_js` for login/web pages)
3. Run `bench build --app arkan_theme`

### New SCSS Partial

1. Create `public/scss/_<name>.scss`
2. Add `@import '<name>';` to `arkan.scss` in the correct position
3. Run `npx sass scss/arkan.scss css/arkan.css --style compressed --source-map`
4. Run `bench build --app arkan_theme`

### New Image/SVG

1. Place in `public/images/` or `public/svg/`
2. Reference as `/assets/arkan_theme/images/<file>` or `/assets/arkan_theme/svg/<file>`
3. Run `bench build --app arkan_theme`

### Adding Boot Data

Edit `boot.py` to add data accessible via `frappe.boot.arkan_theme`:

```python
def boot_session(bootinfo):
    bootinfo.arkan_theme = {
        "version": "16.0.0",
        "my_new_field": "value"
    }
```
...
arkan.config                    // Should show config object

// Check loading module
arkan.loading                   // Should show init, show, remove methods

// Force show splash
arkan.splash.forceShow()

// Check body classes
document.body.classList.toString()  // Should include arkan-day or arkan-night
...
| Dark overlay stuck | Frappe `#freeze` not cleaned up | `arkan.loading._cleanupFrappeFreeze()` or hard refresh |
...
1. **CSS debugging:** Use Elements panel → filter by `arkan` to find all custom styles
2. **JS debugging:** Type `arkan.` in Console to explore the namespace
3. **Network:** Filter by `arkan` to see all theme asset requests
...
- **Variables:** Use `$arkan-*` for compile-time, `var(--arkan-*)` for runtime
...
- JS: `arkan_<module>.js`
- SCSS: `_<feature>.scss` (partials) or `arkan.scss` (master)
...
- [ ] Time-aware theme switches (check `arkan-day`/`arkan-night` class)
...
cd apps/arkan_theme/arkan_theme/public
npx sass scss/arkan.scss css/arkan.css --style compressed --no-source-map

# 2. Build Frappe assets
cd ~/frappe-bench
bench build --app arkan_theme --production
...
1. `boot.py` → `bootinfo.arkan_theme.version`
2. `arkan_theme.js` → `arkan.config.version`
...
The solution is **sidebar-first injection**: each control tries `.body-sidebar-bottom` first, creating sidebar-style markup with `.arkan-sidebar-action` class. If the sidebar isn't found (e.g., during early boot or on mobile), it falls back to the navbar.
...
Frappe v16 uses `.active-sidebar` for the selected navigation item, while v15 used `.selected`. Both are styled to ensure the gold accent bar appears regardless of version.
