...
"""
CAPS Permission Gate for Arkan Theme
...
def has_capability(user=None, capability=None):
    """Check if user has a specific Arkan Theme capability via CAPS."""
    if not user:
    ...
    if not capability.startswith("AT_"):
        capability = f"AT_{capability}"
