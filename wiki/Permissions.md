...
Arkan Theme uses the **CAPS** (Capability-Based Access Control) system for fine-grained permissions.
...
|-----------|-------------------|------------|
<!-- | `AT_manage_xxx` | Access xxx management | الوصول لإدارة xxx | -->
...
| Capability | Description (EN) | الوصف (AR) |
|-----------|-------------------|------------|
<!-- | `AT_approve_xxx` | Approve xxx | الموافقة على xxx | -->
...
|-----------|---------|-------|----------|
<!-- | `AT_view_cost` | `AT Project` | `total_cost` | mask | -->
...
|-----------|--------|------------|
<!-- | `AT_export_reports` | All reports | تصدير التقارير | -->

## Roles — الأدوار

| Role | Capabilities | Description |
|------|-------------|-------------|
<!-- | `AT User` | Basic read access | المستخدم الأساسي | -->

## Jobs/Positions — الوظائف

| Job | Roles Included | Description |
|-----|---------------|-------------|
<!-- | `AT Manager` | User + Admin roles | المدير | -->

## Checking Permissions in Code — فحص الصلاحيات في الكود

```python
from arkan_theme.caps.gate import check_user_capability

# Check capability (throws on failure)
check_user_capability("AT_manage_records", throw=True)

# Check capability (returns bool)
if check_user_capability("AT_approve_records", throw=False):
    approve_record(doc)
```
