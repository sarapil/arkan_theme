# Contributing to Arkan Theme — المساهمة في Arkan Theme
...
```bash
git clone https://github.com/YOUR_USERNAME/arkan_theme.git
cd arkan_theme
```
...
```bash
bench get-app arkan_theme /path/to/your/fork
...
bench --site dev.localhost run-tests --app arkan_theme

# Run specific test — تشغيل اختبار محدد
bench --site dev.localhost run-tests --app arkan_theme --module arkan_theme.tests.unit.test_my_service
```
...
- Update help files in `arkan_theme/help/` for DocType changes
