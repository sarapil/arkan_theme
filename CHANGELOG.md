...
The first public release of ARKAN Theme — a luxury brand theme for ERPNext / Frappe.
Now updated and compatible with Frappe v16 / ERPNext v16.

### Added

#### Core Theme
- **Luxury color system** with 9 core tokens (Gold, Navy, Cream + variants) and 4 semantic colors
- **CSS custom properties** (`--arkan-*`) for runtime theming
- **4 SCSS mixins**: `arkan-glass`, `arkan-card-elevated`, `arkan-gold-glow`, `arkan-focus-ring`
- **Rubik + Poppins** font system (Google Fonts)
- **Time-aware theme** — automatic `arkan-day`/`arkan-night` body classes (6am–6pm boundary)

#### Visual Features
- **Animated Dubai skyline** — Runtime SVG with Burj Khalifa, Burj Al Arab, Dubai Frame, Cayan Tower, and 15+ buildings
- **Water reflections** with animated `<animateTransform>` SVG elements
- **Twinkling window lights** on building silhouettes
- **Canvas effects** — Stars (80/50/25 by viewport), shooting stars (~5–8/min), gold dust particles
...
- **Title observer** — Appends "| ARKAN" to page title via MutationObserver
- **Theme color meta tag** — Sets `<meta name="theme-color">` to navy
- **Boot session data** — `frappe.boot.arkan_theme` with version, brand, logos
...
- **SCSS compilation** required: `npx sass scss/arkan.scss css/arkan.css`

---

## [16.0.0] — 2026

### 🔄 Frappe v16 Migration

Full migration to Frappe v16 / ERPNext v16 compatibility.

#### Changed
- **Build system**: Migrated from `setup.py` to `pyproject.toml` with `flit_core` backend
- **Version**: Bumped from `1.0.0` to `16.0.0` (in `__init__.py`, `boot.py`)
- **Python requirement**: `>=3.14` (was `>=3.10`)
- **URL routing**: All `/app` URLs changed to `/desk` (mobile nav, service worker, PWA manifest)
- **DocType sort_field**: Changed from `"modified"` to `"creation"` (v16 standard)
- **hooks.py**: Removed `app_version` (now read dynamically from `__init__.py`)

#### Added
- **`add_to_apps_screen`** hook for v16 app launcher integration
- **`source_link`** hook pointing to GitHub repository
- **`app_home`** hook set to `/desk`
- **`[tool.bench.frappe-dependencies]`** section in `pyproject.toml`
- **`[tool.bench.dev-dependencies]`** section in `pyproject.toml`
- **Ruff linter** configuration targeting Python 3.14
- **`.gitignore`** file for development/production workflow

#### Removed
- `setup.py` (replaced by `pyproject.toml` with flit_core)
- `requirements.txt` (dependencies now in `pyproject.toml`)
- `app_version` from `hooks.py` (v16 reads version dynamically)
